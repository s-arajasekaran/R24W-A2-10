...
#This is code from A1- specifically from Ajay Rajasekaran with a few modifications
...
#Same def as A1 but added stopword exclusion and tokens must be longer than one character
#also modified to take a string as input
...
#modified to append to existingDict
...
#New function to check sum of all VALID tokens
def checkSum(existingDict, sumList):
    bigSum = 0
    for key in existingDict:
        smallSum = 0
        for c in key:
            smallSum+= ord(c)
        bigSum = bigSum + smallSum * existingDict[key]
    returnVal = True
    if bigSum in sumList:
        returnVal = False
    else:
        sumList.add(bigSum)
    return (sumList, returnVal)
...
#modified to return a formated printable string
